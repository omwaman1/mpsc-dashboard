...
async function processSingleTest(testObj, index, total) {
    ...
    const categoryName = "MPSC Group-C Combined";
...
async function main() {
    console.log("=================================================");
    console.log("=== MPSC GROUP-C FULL RE-SYNC & SOLUTION REPAIR ===");
    console.log("=================================================\n");

    const db = getPool();

    console.log("Fetching test structure for MPSC Group-C...");
    ...
    // Parallel queue worker execution for ALL tests (to repair missing options & solutions)
    async function worker(workerId, queue) {
        while (queue.length > 0) {
            ...
            if (completedCount % 5 === 0 || queue.length === 0) {
                console.log(`\n---> Progress: [${completedCount}/${allTests.length}] test papers synced. Questions updated: ${totalSaved}\n`);
            }
            await sleep(200);
        }
    }

    const queue = allTests.map((t, idx) => ({ test: t, index: idx }));
    const workers = [];
    ...
    console.log("=================================================");
    console.log("=== MPSC GROUP-C FULL RE-SYNC COMPLETE ===");
    console.log(`Total Test Papers Processed: ${completedCount}`);
    console.log(`Total Questions Synced & Repaired: ${totalSaved}`);
    console.log("=================================================");
...
main().catch((err) => {
    console.error("FATAL ERROR in Group-C sync:", err);
    process.exit(1);
});
