...
const SLUG = "maharashtra-forest-guard";
const CONCURRENCY = 8;
...
async function processSingleTest(testObj, index, total) {
    ...
    const categoryName = "Maharashtra Forest Guard (वनरक्षक)";
...
async function main() {
    console.log("=================================================");
    console.log("=== MAHARASHTRA FOREST GUARD PARALLEL SYNC ===");
    console.log("=================================================\n");

    const db = getPool();

    console.log("Fetching test structure for Maharashtra Forest Guard...");
    ...
    if (testsToSync.length === 0) {
        console.log("All Maharashtra Forest Guard test papers are already fully synced!");
        process.exit(0);
    }
    ...
    console.log("=================================================");
    console.log("=== MAHARASHTRA FOREST GUARD SYNC COMPLETE ===");
...
main().catch((err) => {
    console.error("FATAL ERROR in Forest Guard sync:", err);
    process.exit(1);
});
